...
execute store result score @s _skills.cloacking.current_chance run data get entity @e[type=area_effect_cloud, tag=random_uuid, tag=cloacking_chance, limit=1, sort=nearest] UUID[0] 1
...
execute if score @s[nbt=!{ActiveEffects:[{Id:14}]}] _skills.cloacking.current_chance matches 25..54 run effect give @s invisibility 6 1 true

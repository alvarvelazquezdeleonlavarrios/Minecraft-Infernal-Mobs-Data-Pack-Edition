...
#   mob.skills.ender();
execute if entity @s[gamemode=survival] at @s if entity @e[tag=ender, limit=1, sort=nearest, distance=..40, scores={_skills.ender.current_time=..0}] as @e[tag=ender, limit=1, sort=nearest, distance=..40, scores={_skills.ender.current_time=..0}] run function infernal_mobs:skills/ender

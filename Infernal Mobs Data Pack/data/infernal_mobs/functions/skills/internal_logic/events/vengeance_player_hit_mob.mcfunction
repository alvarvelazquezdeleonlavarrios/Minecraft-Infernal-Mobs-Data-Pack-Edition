...
execute if entity @s[gamemode=survival] at @s if entity @e[tag=vengeance, limit=1, sort=nearest, distance=..40, scores={_skills.vengeance.current_time=..0}] as @e[tag=vengeance, limit=1, sort=nearest, distance=..40, scores={_skills.vengeance.current_time=..0}] at @s run function infernal_mobs:skills/vengeance

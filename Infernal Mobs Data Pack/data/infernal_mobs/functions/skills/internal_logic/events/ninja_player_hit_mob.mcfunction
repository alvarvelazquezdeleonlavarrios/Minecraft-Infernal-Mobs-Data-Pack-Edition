...
#   mob.skills.ninja();
execute if entity @s[gamemode=survival] at @s if entity @e[tag=Infernal_Mob, tag=ninja, limit=1, sort=nearest, distance=..40, scores={_skills.ninja.current_time=..0}] as @e[tag=ninja, limit=1, sort=nearest, distance=..40, scores={_skills.ninja.current_time=..0}] at @s run function infernal_mobs:skills/ninja
...
advancement revoke @s only infernal_mobs:ninja/player_hit_mob

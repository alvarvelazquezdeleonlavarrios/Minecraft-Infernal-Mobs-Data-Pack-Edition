...
execute if score @s _aecDthD.xpOrbsA matches 1.. run summon experience_orb ~ ~ ~ {Value:10}

#------- Decreases the experience orbs amount. -------
# aec_death_detector.experience_orbs_amount--;
scoreboard players remove @s _aecDthD.xpOrbsA 1

#------- Recursively summons the missing experience orbs for the mob. -------
# if (aec_death_detector.experience_orbs_amount >= 1):
#   aec_death_detector.generate_experience_orbs();
execute if score @s _aecDthD.xpOrbsA matches 1.. run function infernal_mobs:gameplay/death_detection/generate_experience_orbs

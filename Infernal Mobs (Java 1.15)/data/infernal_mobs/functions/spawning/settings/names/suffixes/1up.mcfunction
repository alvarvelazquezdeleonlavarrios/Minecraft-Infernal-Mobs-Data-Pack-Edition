...
scoreboard objectives add _tmp.rndSuf.1up dummy
scoreboard players set @s _tmp.rndSuf.1up -1
...
execute store result score @s _tmp.rndSuf.1up run data get entity @e[type=area_effect_cloud, tag=random_uuid, tag=random_suffix_1up, limit=1] UUIDLeast 0.00000000023283064365386962890625
scoreboard players operation @s _tmp.rndSuf.1up %= $Constants _const.3
...
execute if score @s _tmp.rndSuf.1up matches 0 run data modify block ~ ~ ~ Text2 set value '{"text":" ofRecurrence","interpret":true}'
execute if score @s _tmp.rndSuf.1up matches 1 run data modify block ~ ~ ~ Text2 set value '{"text":" theUndying","interpret":true}'
execute if score @s _tmp.rndSuf.1up matches 2 run data modify block ~ ~ ~ Text2 set value '{"text":" oftwinLives","interpret":true}'
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
scoreboard objectives remove _tmp.rndSuf.1up

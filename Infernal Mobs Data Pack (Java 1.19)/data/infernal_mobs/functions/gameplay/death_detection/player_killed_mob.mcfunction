...
execute as @e[type=area_effect_cloud, tag=infernal_mob_death_detector, scores={_aec_death_detector.is_binded_to_mob=1}] at @s unless entity @e[tag=Infernal_Mob, limit=1, sort=nearest, distance=..0.3] run function infernal_mobs:gameplay/death_detection/generate_loot

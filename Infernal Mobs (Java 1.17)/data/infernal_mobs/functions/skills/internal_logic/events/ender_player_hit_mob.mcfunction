...
#   mob.skills.ender();
execute if entity @s[gamemode=survival] at @s if entity @e[tag=ender, limit=1, sort=nearest, distance=..40, scores={_sk.end.crtT=..0}] as @e[tag=Infernal_Mob, tag=ender, limit=1, sort=nearest, distance=..40, scores={_sk.end.crtT=..0}] at @s run function infernal_mobs:skills/ender

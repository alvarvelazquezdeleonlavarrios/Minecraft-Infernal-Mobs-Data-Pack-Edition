...
scoreboard players set @s _ntrlSpwn.vrtChc -1
...
execute store result score @s _ntrlSpwn.vrtChc run data get entity @e[type=area_effect_cloud, tag=random_uuid, tag=random_variant, limit=1] UUIDLeast 0.00000000023283064365386962890625
scoreboard players operation @s _ntrlSpwn.vrtChc %= $Constants _const.10000
...
#   mob.setInfernal();
execute if score @s _ntrlSpwn.vrtChc matches 0..65 run function infernal_mobs:spawning/infernal

#------- This will be an ultra variant (2.0% of chance). -------
# if (mob.natural_spawning_variant_chance = [66,265]):
#   mob.setUltra();
execute if score @s _ntrlSpwn.vrtChc matches 66..265 run function infernal_mobs:spawning/ultra

#------- This will be a rare variant (10.0% of chance). -------
# if (mob.natural_spawning_variant_chance = [266,1265]):
#   mob.setRare();
execute if score @s _ntrlSpwn.vrtChc matches 266..1265 run function infernal_mobs:spawning/rare

#------- This will NOT be an Infernal Mob (87.33% of chance). -------
# if (mob.natural_spawning_variant_chance = [1266,9999]):
#   mob.tags.Add("Common_Mob");
execute if score @s _ntrlSpwn.vrtChc matches 1266..9999 run tag @s add Common_Mob

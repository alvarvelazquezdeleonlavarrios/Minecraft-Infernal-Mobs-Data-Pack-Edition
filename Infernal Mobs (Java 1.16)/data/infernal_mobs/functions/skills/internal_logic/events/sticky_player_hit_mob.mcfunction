...
#   mob.skills.sticky();
execute if entity @s[gamemode=survival, nbt={SelectedItem:{}}] at @s if entity @e[tag=sticky, limit=1, sort=nearest, distance=..6, scores={_sk.sticky.crtT=..0}] as @e[tag=Infernal_Mob, tag=sticky, limit=1, sort=nearest, distance=..6, scores={_sk.sticky.crtT=..0}] at @s run function infernal_mobs:skills/sticky

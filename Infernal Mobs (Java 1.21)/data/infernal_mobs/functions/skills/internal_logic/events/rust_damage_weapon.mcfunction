...
scoreboard objectives add _tmp.rust.selected_item_current_damage dummy
scoreboard players set @s _tmp.rust.selected_item_current_damage 0
...
execute store result score @s _tmp.rust.selected_item_current_damage run data get entity @s SelectedItem.components."minecraft:damage"
scoreboard players operation @s _tmp.rust.selected_item_current_damage += $Constants _const.6

#------- Sets the new damage dealt value for the villager's mainhand item -------
# villager.HandItems["right_hand"].damage = player._tmp_damage;
execute store result entity @e[type=villager, tag=rust_player_hit_mob, limit=1, sort=nearest] HandItems[0].components."minecraft:damage" int 1 run scoreboard players get @s _tmp.rust.selected_item_current_damage
...
scoreboard objectives remove _tmp.rust.selected_item_current_damage
